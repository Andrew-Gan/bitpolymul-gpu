...
while(line):
	...
	if '<<' in line:
		nThread = numbers[6] - numbers[3]
	else:
		nThread = numbers[2] - numbers[1]
	...
	if not divisible:
		...
		if '<<' in line:
			func_str += 'xor_gpu<<<{}, {}>>>(poly, offset+({}<<{})-{}, '.format(nBlock, nThread, numbers[4], numbers[5], numbers[6])
			func_str += str(numbers[7:])[1:-1]
		else:
			func_str += 'xor_gpu<<<{}, {}>>>(poly, offset-{}, '.format(nBlock, nThread, numbers[2])
